...
$msg="";
$msg.="
     <form name=\"\" method = \"POST\" action=\"".base_url()."admin/admin_booking/unpaid_agency/N/$sid/$rows->compid\">";
	  if($rows->compid == ""){
	      $msg.="  <table>
		   		<tr>
					<td>".anchor('admin/sales_report/allsalesrpt_by_agency/Y/'.$billno,"Paid")."</td>
					<td> &nbsp; </td>
					<td>".anchor('admin/sales_report/allsalesrpt_by_agency/N/'.$billno,"Unpaid")."</td>
				</tr>
        ...
        $cname=$rows->comp_name;
	     $msg.="  <table>
		   		<tr>
					<td>".anchor('admin/sales_report/allsalesrpt_by_agency/Y/'.$rows->compid,"Paid")."</td>
					<td> &nbsp; </td>
					<td>".anchor('admin/sales_report/allsalesrpt_by_agency/N/'.$rows->compid,"Unpaid")."</td>
				</tr>
					  ...
					  if($rows->paid =="N"){
					     $sales_category=$this->session->userdata('sales_category');
                            if($sales_category == 'ad'){
								$msg.="
                                    [".anchor('admin/sales_report/allsalesrpt_by_agency_pay/N/'.$billno.'/'.$rows->sales_id,"Pay Now")."]
                                    ";
...
<script type=\"text/javascript\" src=\"".base_url()."res/js/calendar.js\"></script>
        <br><table align=\"center\" style=\"border:1px solid #DDDDDD;\">
            <tr><td>Ticket No:</td><td>$sid</td><td>Agency Name:</td><td>$rows->comp_name</td></tr>
