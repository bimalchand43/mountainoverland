...
//print_r($seatnumbe);
  ...
  foreach($query2 as $rowpseat){
   $pseatnum = $rowpseat->seat_no;
  ...
  $x=0;
 if($cancelled == "C"){
   foreach($query2 as $rowseat){
   $indseat = explode(",",$rowseat->seat_no);
   $rates = $rowseat->rate;
  }
  for($x=0;$x < sizeof($indseat);$x++){
  	$arrsel.="arrseatsel[$x]=".$indseat[$x]."\n";
...
function showseat(){
    ...
    //alert(tseat);
	totfare=$rates * arrseatsel.length;
...
function showSeatno(seatno,fare){
	...
	var isselect ='http://www.mountainoverland.com/res/images/selectseat.jpg';
//    var isselect ='http://www.mountainoverland.com/res/images/selectseat.jpg';
		 ...
		 for ($i=1; $i<=5; $i++)
		  {
							...
							for ($j=1; $j<=9; $j++){
								...
								if($i==3 and ($j>= 1 and $j <= 8)){
                                    ...
                                    if($cancelled == "C"){
                                    for($p=0;$p<sizeof($indseat);$p++){

										if($indseat[$p] == $arrseatno[$k]){
											$bookedseat1=$indseat[$p];
											break;
										}else{
											$bookedseat1 = 0;
										}
									...
									//echo $bookedseat;
                                    $bookedval=0;
									if($bookedseat1 > 0 || $bookedseat > 0){
                                        if($bookedseat1 > 0){
                                          for($y=0;$y<sizeof($indseat);$y++)
                                          {
                                              if($indseat[$y] == $arrseatno[$k]){
                                                  $bookedval=1;
                                                  break;
                                              }
                                          }
                                        ...
                                        if($bookedval == 1){
  										    ...
  										    $showseat.="<a href=\"#\" calss=\"vtip\" title=\"Seat No: ".$arrseatno[$k]."\"align=\"middle\">
  										    <img id=\"img$arrseatno[$k]\" src=\"".base_url()."res/images/selectseat.jpg\" onClick=\"showSeatno($arrseatno[$k],$fare);\" /></a>$arrseatno[$k]
								            </td>";

                                        }else{
                                            $showseat.="<img id=\"img$arrseatno[$k]\" src=\"".base_url()."res/images/seatsel.jpg\" />$arrseatno[$k]
								            </td>";
                                        }
                                    ...
                                    if($cancelled == "C"){
                                        ...
                                        if($cancelled == "C"){
									        $showseat.="<td align=\"center\" valign=\"middle\"><a href=\"#\" calss=\"vtip\" title=\"Seat No: ".$arrseatno[$k]."\"align=\"middle\">
...
if($cancelled == "C"){
    $showseat.="<script language=\"javascript\">
          showseat();
 </script>
";
}
...
$i=0;
 ...
 foreach($query1 as $rows)
  ...
  //echo $id;
   ...
   foreach($query3 as $rowmaster){

   }
$mcheckedM="";
$mcheckedF="";
$mcheckedO="";
$mgender=$rowmaster->gender;
if($mgender == 'M') $mcheckedM='selected';
if($mgender == 'F') $mcheckedF='selected';
if($mgender == 'O') $mcheckedO='selected';
?>
...
<table class="tbl_border" width="100%" style="clear:both;">
	...
	<tr>
    	...
    	<td> <input type="text" name="mast_fname" value="<?php echo $rowmaster->name_on_ticket; ?>" size="50" /></td>
        <td colspan="2">&nbsp; </td>
    </tr>
    <tr>
        	<td class="lebl"> Address : </td>
            <td><input type="text" name="address" value="<?php echo $rowmaster->contact_address; ?>" size="50"/> </td>
            <td class="lebl"> Email ID : </td>
            <td><input type="text" name="email" value="<?php echo $rowmaster->email; ?>" size="25"/> </td>

        </tr>

        <tr>
        	<td class="lebl"> Phone No. : </td>
            <td><input type="text" name="phone" value="<?php echo $rowmaster->phone; ?>" size="25"/> </td>
            <td class="lebl">Mobile No. :</td>
             <td><input type="text" name="mobile" value="<?php echo $rowmaster->mobile; ?>" size="25"/> </td>
        </tr>
         <tr>
        	<td class="lebl"> Nationality : </td>
            <td><input type="text" name="mnationality" value="<?php echo $rowmaster->nationality; ?>" size="25"/> </td>
            <td class="lebl">Passport/ID No :</td>
             <td><input type="text" name="midno" value="<?php echo $rowmaster->idno; ?>" size="25"/> </td>
        </tr>
        <tr>
        	<td class="lebl"> Age : </td>
            <td><input type="text" name="mage" value="<?php echo $rowmaster->age; ?>" size="25"/> </td>
            <td class="lebl">Gender :</td>
             <td><select name="mgender">
                <option value="M" <?php echo $mcheckedM; ?> >Male </option>
                        <option value="F" <?php echo $mcheckedF; ?> >Female</option>
                        <option value="O" <?php echo $mcheckedO; ?> >Other</option>
                </select></td>
        </tr>
</table>
...
<div id="descr"><table class="ltable" width="100%">
        <tr bgcolor="#CCCCCC">
        	<td colspan="7"> Passanger List Details & Contact Information </td>
        </tr>
        <tr>
        	<th>&nbsp;  </th>
            <th> Fullname </th>
		...
		for($i=1; $i<= $tpass; $i++)
		{
			...
			if(sizeof($query4) >0){
				foreach($query4 as $rowplist){

				if($seatnum[$k] == $rowplist->seatno){
				 $fname=$rowplist->fullname;

				 $age=$rowplist->age;
                 $nationality=$rowplist->nationality;
                 $idno = $rowplist->idno;
                  $gender=$rowplist->gender;
				 if($gender == 'M') $checkedM='selected';

				  if($gender == 'F') $checkedF='selected';
                  if($gender == 'O') $checkedO='selected';
		...
		?>
                <tr>
                    ...
                    <td> <input type="text" name="fname[]" value="<?php echo $fname;?>" size="50"> </td>
                    <td> <input type="text" name="nationality[]" value="<?php echo $nationality;?>" size="12"> </td>
                    <td> <input type="text" name="idno[]" value="<?php echo $idno;?>" size="16"> </td>
                    <td> <select name="gender[]">
                        <option value="M" <?php echo $checkedM; ?> >Male </option>
                        <option value="F" <?php echo $checkedF; ?> >Female</option>
                        <option value="O" <?php echo $checkedO; ?> >Other</option>
                        </select></td>
                    <td> <input type="text" name="age[]" value="<?php echo $age;?>" size="5"></td>  
...
<table class="tbl_border" width="100%">
	<tr align="right">
            ...
            <input type="hidden" name="pseat" value="<?php echo $pseatnum ?>" />
