...
$msg.="
     ...
     <table border=0 cellspacing=0 cellpadding=5 class=\"ltable\" style=\"border:1 solid #DDDDDD;\" width=\"100%\">
        ...
        $msg.="
        <tr class=\"$rc\">
            <td align=\"center\">".anchor('admin/sales_report/periodicSalesReport/yearly/'.$rows->tyear,"$rows->tyear")."</td>
            ...
            <td align=\"center\">[".anchor('admin/sales_report/periodicSalesReport/yearly/'.$rows->tyear,"View")."]</td>
        </tr>
        ";
        $i++;
              ...
              $rc="lcb";
         	  if($i%2==1) $rc="dcb";
                        $msg.="
                                 <tr class=\"$rc\">
                                    <td> ".$rows->yr."</td>
        				            <td>".anchor('admin/sales_report/periodicSalesReport/monthly/'.$rows->yr.'/'.$rows->mn,"$rows->mn")."</td>
                                    <td align=\"center\">".$rows->paid_tick."</td>
									<td align=\"center\">".number_format($rows->paid_amt,2)."</td>
									<td align=\"center\">".$rows->unpaid_tick."</td>
									<td align=\"center\">".number_format($rows->unpaid_amt,2)."</td>
                                    <td align=\"right\">".number_format($totalbill,2)."</td>
                                    <td>[".anchor('admin/sales_report/periodicSalesReport/monthly/'.$rows->yr.'/'.$rows->mn,"View")."]</td>
        		                </tr>
                            ";
...
<script type=\"text/javascript\" src=\"".base_url()."res/js/calendar.js\"></script>
			<table border=0 cellspacing=0 cellpadding=5 class=\"ltable\" style=\"border:1 solid #DDDDDD;\" width=\"100%\">
              <h3 style=\"background-color: #DEDEFC; width:98%; text-align: left;\">$stitle</h3>
                    <tr><td colspan=\"9\"><form name=\"dsales\" method=\"post\" action=\"".base_url()."admin/sales_report/periodicSalesReport/datewise\">
				  ...
				  $j=$i+1;
				   $rc="lcb";
         	       if($i%2==1) $rc="dcb";
                        $msg.="
                                 <tr class=\"$rc\">	
										<td align=\"center\">".anchor('admin/sales_report/periodicSalesReport/daily/'.$rows->dt,"$rows->dt")."</td>
										<td align=\"center\">".$rows->paid_tick."</td>
										<td align=\"center\">".number_format($rows->paid_amt,2)."</td>
										<td align=\"center\">".$rows->unpaid_tick."</td>
										<td align=\"center\">".number_format($rows->unpaid_amt,2)."</td>
										<td align=\"right\">".number_format($totalbill,2)."</td>
										<td align=\"center\">[".anchor('admin/sales_report/periodicSalesReport/daily/'.$rows->dt,"View")."]</td>
									</tr>
								";
